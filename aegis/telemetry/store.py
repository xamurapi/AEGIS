...
from aegis.config import TELEMETRY_DIR, TELEMETRY_FLUSH_ROWS, TELEMETRY_FLUSH_SECONDS, TELEMETRY_MAX_ROWS
logger = logging.getLogger('aegis.telemetry')
SCHEMA_VERSION = 2
_SAFE_NAME = re.compile('[^A-Za-z0-9._-]')
_MAX_NAME = 120
...
class Series:
    ...
    def rows(self) -> list[dict]:
        return [{'tick': t, 'tick_end': e, 'value': v, 't': ts, 'n': n, 'tags': tg} for t, e, v, ts, n, tg in zip(self.ticks, self.ends, self.values, self.times, self.counts, self.tags)]

class Telemetry:
    """Buffered writer + reader for metric series."""

    def __init__(self, directory: Path | None=None, max_rows: int=TELEMETRY_MAX_ROWS, flush_rows: int=TELEMETRY_FLUSH_ROWS, flush_seconds: float=TELEMETRY_FLUSH_SECONDS):
    ...
    def record(self, metric: str, value, tick: int=0, tags: dict | None=None) -> bool:
        ...
        if numeric != numeric or numeric in (float('inf'), float('-inf')):
            self.dropped += 1
            return False
        row = {'tick': int(tick), 'value': numeric, 't': round(CLOCK.now(), 3), 'n': 1}
        if tags:
            row['tags'] = {str(k): tags[k] for k in sorted(tags)}
        self._buffer.setdefault(self.safe_name(metric), []).append(row)
        self._pending += 1
        if self._pending >= self._flush_rows or CLOCK.now() - self._last_flush >= self._flush_seconds:
            self.flush()
        return True

    def record_many(self, metrics: dict, tick: int=0, tags: dict | None=None) -> int:
        """Record a whole batch; returns how many were accepted."""
        return sum((1 for name, value in sorted(metrics.items()) if self.record(name, value, tick, tags)))
    ...
    def flush(self) -> int:
        ...
        for name, rows in sorted(self._buffer.items()):
            ...
            self._rows_on_disk[name] = self._count_file(path) if known is None else known + len(rows)
    ...
    def _merge(rows: list[dict]) -> dict:
        ...
        return {'tick': int(rows[0].get('tick', 0)), 'tick_end': int(rows[-1].get('tick_end', rows[-1].get('tick', 0))), 'value': round(value, 6), 't': round(float(rows[0].get('t', 0.0)), 3), 'n': total_n}

    def series(self, metric: str, window: int | None=None, include_buffer: bool=True) -> Series:
    ...
    def status(self) -> dict:
        return {'schema_version': SCHEMA_VERSION, 'directory': str(self._dir), 'metrics': len(self.metrics()), 'records_written': self.records_written, 'pending': self._pending, 'flushes': self.flushes, 'downsamples': self.downsamples, 'dropped': self.dropped, 'max_rows_per_metric': self._max_rows}